...
/**
 * Handling all types of comparison provided by application.
 * Class deals with surface comparison
 * 
...
public class SurfaceComparisonProcessing {
    ...
    /**
     * Aligns two faces, main and secondary (compF), using ICP algorithm.
     * User can decided how many vertices will be used for alignment (hence not 
     * all vertices will need to be considered when computing parameters for alignment)
     * 
     * Does not create local copies of either main or secondary face
     * 
     * @param mainF - KdTree representation of main face to which secondary face will be aligned
     * @param compF - point cloud representation of secondary face which will be aligned to main face
     * @param numberOfIterations - number of iterations performed during ICP algorithm
     * @param scale - whether to use scale during ICP
     * @param error - plausible error rate, to stop ICP before numberOfIterations is reached
     * @param method - method of undersampling
     * @param t - type of undersampling (samples, percentage -- so far only valid for Random Undersampling)
   ...
   /**
    * Aligns set of faces (compFs) to main face using ICP algorithm.
    * Can decide how many vertices will be used to compute alignment parameters. Number of vertices
    * used is same for each face in set (unless number is larger than number of vertices, in that case
    * all vertices are used). 
    * Uses pair alignment; iteratively aligns each face to main face.
    * 
    * Creates local copies of aligned faces (compFs).
    * 
    * @param mainF - KdTree representation of main face
    * @param compFs - list containing Files with all the faces to be aligned to main face
    * @param numberOfIterations - number of iterations performed during ICP algorithm
     * @param scale -  whether to use scale during ICP
    * @param error - plausible error rate, to stop ICP before numberOfIterations is reached
    ...
    */
    ...
    /**
     * Compare main face with average face created from set of faces. As
     * the result of comparison list of Hausdorff distances are returned.
     * Allows to use relative, or absolute coordinates.
     * 
     * Normals of average face are recomputed in process
     * 
     * @param mainF - KdTree representation of main face
     * @param avarage - Model class containing information about average face
     * @param useRelative - defines whether algorithm should computed relative or absolute values of HD
     * @return list containing resluts of comparison, as float number representing hausdorff metric for each point in average face
     */
    ...
    /**
     * Given the template, method alignes all faces in 'compFs' to template and computes new average face. 'numberOfBatchIterations' is number
     * of average faces created in process. Each face from 'compFs' is aligned to template by performing 'numberOfICPiteration' ICP iterations.
     * It is possible to undersample the model by only performing ICP on 'usedSamples' % of vertices.
     * 
     * @param template - template face to be used to create average faces
     * @param compFs - list of faces to be aligned to template face and to compute new average faces from
     * @param numberOfBatchIterations - number of average faces to be created
     * @param numberOfICPiteration - number of ICP iterations to be performed for each face in compFs during each ICP alignment
     * @param scale - whether to use scale during ICP
     * @param error - error rate, for ICP algorithm, if change between two iterations is smaller than error, ICP stops
     * @param m - method of undersampling
     ...
     * @throws FileManipulationException if the method was unable to write down temporary files on disk
     */
    ...
    /**
     * Adds translation computed in 'list1' to sum of all translations in 'trans'.
     * @param trans list containing sum of all computed translations, size equals to number of template vertices
     * @param list1 list with translations for each vertex of trans computed from single compF
     */
    ...
    /**
     * Computes average translation, based on number of faces that are being compared and applies translation to template
     * creating average face.
     * 
    ...
    /**
     * Computes raw visual results of comparison of each face in 'compFs' to 'template'. Returns matrix of size template.size X compFs.size
     * as it computes a single value for each pair template-compFs.element, so that each vertex of template has compFs.size values asigned to it.
     * These values are then used in computation of variation method and final result (such as RMS, min, max etc.)
     * 
     * @param template - average face to which all faces in 'compFs' will be compared to
     * @param compFs - faces to be compared to template
     * @param useRelative - should the result be relative distance (takes into consideration whether point is in front of behind template) or absolute (classic Euclidean distance)
     * @param method - method used for comparison of models
     * @param metric - ICP metric used for comparison
     * @return matrix of size template.size X compFs.size containing raw results to be either analyzed on their own, or compute numerical results from
     * @throws FileManipulationException if method couldn't save temporary results on disk
     */
    ...
    /**
     * Computes numerical results for batch comparison based on 'varianceMethod' picked, for each model in 'models'. Creates matrix of models.size X models.size
     * as such computes difference of each model in 'models' in respect to each model in 'models'. It is possible to only use certain amount of computed numerical result values
     * by setting up appropriate value of 'thresh', in range [0, 1].
     * 
     * @param models models to be compared to each other
     * @param varianceMethod variance method to compute final results with
     * @param useRelative whether relative distance should be employed (with sign), or whether classical Euclidean distance should be used instead 
     * @param thresh - allows to only use certain amount of coputed numerical result values, by setting parameter in range of [0, 1]
     * @param auxiliaryResultsFile BatchComparison processing class to store computed results to
     * @return matrix of size models.size X models.size contraining numerical result for each-to-each comparison
     */
    ...
    /**
     * Returns string representation of computed numerical results so it can be displayed in GUI table
     * 
     * @param results computed numerical results to be displayed
     * @param varianceMethod variance method used, this parameter only serves to inform user of the method they picked for final numerical results 
    ...
    /**
     * Computes variance for single comparison numerical results between 2 faces, resulting into one value, based on picked variance method.
     * 
     * @param results raw results to compute variation from
     * @param varianceMethod method to be used to compute final variation
     * @param useRelative whether to use relative distance (with sign) or classical, Euclidean, distance
     * @return single number representing final variance computed from 'results' based on 'varianceMethod'
     */
    ...
    /**
     * Creates list containing variation computed for every vertex of template from tuple of pre-computed visual comparison values. 
     * Serves mainly for computing visual results.
     * 
     * @param resultMatrix matrix of size numberOfTemplateVertices X numberOfComparedModels containing raw visual comparison data
     * @param varianceMethod method to be used to compute variance for each vertex of average face
     * @param useRelative whether to use relative distance (with sign) or classical, Euclidean, distance
     * @return list containing variation computed for every vertex of template from tuple of pre-computed visual comparison values
     */
    ...
    * 
    * @param precomputedResultsFile folder containing CSV files with pre-computed data
    * @param varianceMethod variance method to be used for recomputing numerical results
    * @param numOfModels number of models that were compared
    * @param thresh allows to only use certain amount of pre-computed values, thresh should be in range [0, 1]
    * @param useRelative whether to use relative distance (with sign) or classical, Euclidean, distance
    * @return list containing recoputed numerical results, based on given parameters
    */
   ...
   /**
    * Loads and returns uncomputed results of comparison between two faces, of index numOfMainModel and numOfSecondaryModel. 
    * 
    * @param precomputedResultsFile - directory where aux results are stored
    * @param numOfModels - total number of models with which comparison computes
    * @param numOfMainModel - index of main model
    * @param numOfSecondaryModel - index of secondary model
    * @param useRelative - whether to use signed distance or not
    * @return list of aux results for comparison of numOfSecondaryModel to numOfMainModel
    */
    ...
    * @param compF faces to be used to compute parameters of new average face from
     * @param metric - ICP metric used for registration
    */
    ...
    */
    ...
    /**
     * Recomputes normals of given mesh. Useable when new average face is created.
     * Does not recompute normals if number of vertex normals in mesh is larger than 
     * number of vertices (very likely this is an error in mesh)
     * 
