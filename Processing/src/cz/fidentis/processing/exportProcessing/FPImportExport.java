...
/**
 * Class handles import and export of Feature Points, whether they are computed or loaded.
 * 
...
public class FPImportExport {
    ...
    /**
     * Imports points from disk, in one of supported formats. Right now it is able to load .pp, .fp and .csv files.
     * 
     ...
     * @return - list of loaded points as FPModel class, where each FPModel corresponds to one model and its FP, or null if no files were picked to load
     */
    ...
    /**
     * When model is loaded, it is being centralized, so that the center of the model is
     * at coordinates [0,0,0]. Same transformations need to be applied to loaded FP for them to
     * display correctly on model. This method takes list of all loaded FP for each model and list
     * of all models and pair them based on the name of model stated in FpModel class.
     * Once the pair between FpModel and Model is found, all FPs in FpModel are transformed based
     * on Model's centralization. If there is FpModel that has no model associated loaded, no transformations
     * are applied to FpModel.
     * 
    ...
    /**
     * Saves given points to one of supported formats. So far .pp, .fp and .csv are supported.
     * In order for points to be saved in specific format, user has to give name of file in save window with
     * specific file extension (e.g. if you want to save file as .csv you have to type <file_name>.csv to dialog.
     * If no extension was given by user while picking save path, file is saved as .fp
     * 
     * @param tc - GUI top component, usually window from which save points option was called
    ...
    /**
     * Turn list of FacialPoints to FpModel. If points, or modelName are null, computation is stopped.
     * Does not create copy of Facial Points.
     * 
     * @param points - list of Facial Points
     * @param modelName - name of the model for which the FpModel is being created
     * @return new FpModel with given points and model name, or null if either of parameters is null
     */
    ...
    /**
     * Export points for 1:1 mode. If points were computed in FIDENTIS, points are reverted back
     * to position before models were aligned to model in Frankfurt position. In all cases, points
     * are decentralized based on centralization transformations of model they are associated with.
     * 
     * @param tc - GUI top component, usually window from which point exporting was initialized
     ...
     * 
     * @param tc - GUI top component, usually window from which point exporting was initialized
     * @param data - OneToManyComparison class storing all computed data from 1:N mode
     * @param mainFP - Feature points for main face
     * @param mainModel - main model
     */
