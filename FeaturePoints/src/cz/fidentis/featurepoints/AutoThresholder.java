 ...
 * (http://pacific.mpi-cbg.de/wiki/index.php/Auto_Threshold) by G.Landini at
 * bham dot ac dot uk.
 */
public class AutoThresholder {
    ...
    int Intermodes(int[] data) {
        ...
        while (!bimodalTest(iHisto)) {
            ...
            iHisto[data.length - 1] = (iHisto[data.length - 2] + iHisto[data.length - 1]) / 3; //0 outside
            iter++;
            if (iter > 10000) {
                threshold = -1;
    ...
    int Li(int[] data) {
        ...
        mean = 0.0;
        for (int ih = 0 + 1; ih < data.length; ih++) //0 + 1?
        {
    ...
    int MinErrorI(int[] data) {
        ...
        while (threshold != Tprev) {
            ...
            if (Double.isNaN(temp)) {
                System.out.println("MinError(I): NaN, not converging.");
                threshold = Tprev;
            } else {
    ...
    int Otsu(int[] data) {
        ...
        double S, L = data.length; // The total intensity of the image
        ...
        for (k = 1; k < L - 1; k++) { // No need to check endpoints k = 0 or k = L-1
            ...
            if (denom != 0) {
                ...
                num = ( N1 / N) * S - Sk; 	// Maximum value of num =  data.length - 1*N = approx 8E7
                BCV = (num * num) / denom;
            } else {
                BCV = 0;
            }
